...
**Trigger-analysis workbook — corrected denominator + forward returns (branch `trigger-denominator-ex-spcx`, 2026-09-03).** Analysis layer (`tools/transcripts/`, `analysis/`), Baker/Atreides. `analysis/trigger_analysis.xlsx` is now **17 coherent sheets** on the **thesis-investable-equity (ex-IPO-reclass) denominator**: Summary · Performance · 6 Baker context · 3 Leo context · 6 trigger sheets. Key change: AI-signal triggers (`build_13f_analysis.py`) and all context sheets (`build_trigger_workbook.py`) now weight against COMMON equity **excluding IPO-reclassification tickers** (`IPO_RECLASS_TICKERS = {SPCX, CBRS}`) instead of total book — SpaceX + Cerebras entered the 13F via 2026 IPOs (~$5.36B, not market buys) and were compressing every AI subtheme below the fixed 4%/2% thresholds. Weights sum to 100% ex-reclass; reclass names shown as `[memo]` rows. NEW: `tools/transcripts/trigger_forward_returns.py` (filing-anchored buy-and-hold forward returns at 1m/1q/6m/1y/2y vs SMH+SPY → `analysis/trigger_forward_returns_ex_spcx.{csv,md}` + the Performance sheet). Ex-reclass net deployment = **−$1.35B (net seller)** vs $4.00B as-filed. Backtest read: beats SPY at all horizons, ~in line with SMH (AI-beta); AI_BASKET_RAMP is the only trigger that beats SMH consistently. `mypy` clean. Full detail: `analysis/context_sheets_implementation_notes.md` (iterations 1–6). **Not yet merged to main.**
